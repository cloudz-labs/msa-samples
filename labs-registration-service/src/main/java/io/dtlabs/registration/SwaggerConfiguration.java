...
public class SwaggerConfiguration implements ApplicationContextAware {
	...
	public Docket localeApi() {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("io.dtlabs")).build()
				.apiInfo(getApiInfo()).consumes(getConsumes())
				.produces(getProduces());
	}

	private ApiInfo getApiInfo() {
		return new ApiInfoBuilder().title(applicationContext.getId()).description("Application description.")
				.termsOfServiceUrl("약관").license("라이센스").licenseUrl("라이센스 정보 URL")
				.version("1.0.0").build();
	}
